...
bool Object::Collide(Ball* _ball)
{
	...
	case UP: // banh đi len 
	{
		if (_ball->getPos().x < _x - _ball->R() || _ball->getPos().x > _x + _w + _ball->R() || _ball->getPos().y < _y - _ball->R())
		{
			return false;
		}
	...
	case UPLEFT: // banh đi len sang trái 
	{
		...
		//cham be ngang thanh truot
			...
			&& _ball->getPos().y - _ball->R() < _y + _h - epsilon && _ball->getPos().y >= _y)
		{
	...
	{
		if (_ball->getPos().x < _x - _ball->R() || _ball->getPos().x > _x + _w + _ball->R() || _ball->getPos().y > _y + _h + _ball->R())
		{
			return false;
		}
		if (_ball->getPos().x >= _x && _ball->getPos().x <= _x + _w
	...
	case DOWNLEFT: // banh đi xuống sang trái 
	{
		...
		//cham be ngang thanh truot ben phai
			...
			&& _ball->getPos().y + _ball->R() > _y + epsilon
