...
#define WIDTH 40 // chiều rộng màn hình mặc định
#define HEIGHT 30 // chiều cao màn hinh mặc định
...
void Pong::updateBall()
{
	...
	else
	{
		...
		{
			if (this->_ball->impactWall(this->_width, this->_height)) // kiểm tra va chạm với thanh trượt dưới 
			{
				// tượng tự trên
				this->_ball->draw(); // vẽ 
				this->_ball->clear(); // xóa
...
void Pong::updatePaddle()
{
	...
	this->_playerBottom->draw(); // vẽ
	this->_playerTop->draw(); // vẽ 
}
...
void Pong::runGame()
{
	...
	Paddle p1(this->_width / 2, 1, 5, 0, this->_width, NONE); // tạo thanh trượt trên
	this->_playerTop = &p1; // trỏ tới thanh trượt trên

	Paddle p2(this->_width / 2, this->_height - 1, 5, 0, this->_width, NONE); // tạo thanh trượt dưới 
	this->_playerBottom = &p2; // trỏ tới thanh trượt dưới

	Ball b(this->_width / 2, this->_height / 2, SPEED, DOWN); // tạo trái banh
	...
	this->_playerTop->draw(); // vẽ thanh trượt trên 
	this->_playerBottom->draw(); // vẽ thanh trượt dưới
	this->_ball->draw(); // vẽ trái banh
	...
	while (this->_game_state == PLAYING) // nếu trạng thái là đang chơi thì trò chơi được bắt đầu
	{
		...
		this->updateGame(); // cập nhật các vật thể
...
Pong::Pong(int width, int height, gamestate game_state)
{
	this->_score1 = 0; // gán điểm người chơi 1 băng 0 
	this->_score2 = 0; // gán điểm người chơi 2 bằng 0
	this->_width = WIDTH; // gán chiều rộng màn hình chơi 
	this->_height = HEIGHT; // gán chiểu cao màn hình chơi
...
void Pong::start()
{
	...
	int calculated_pos_bottom = DEFAULT_POS; // vị trí mặc định của thanh trượt dưới khi tự động di chuyển 
	int calculated_pos_top = DEFAULT_POS; // vị trí mặc định của thanh trượt trên khi tự động di chuyển
	...
	while (this->_game_state == INTRO && !(GetAsyncKeyState(VkKeyScan('F') && GetAsyncKeyState(VkKeyScan('f'))))) // nếu trạng thái là INTRO - phần giới thiệu và người dùng không nhấn 'F' thì tiếp tục chạy màn hình tự chơi
	{
		...
		this->updateGame(); // cập nhật vật thể 
...
int Pong::calculatePosBottom(int& dest_pos)
{
	...
	if (dest_pos == DEFAULT_POS) // nếu đang là vị trí mặc định
	{
		...
		if (ball_pos.y == this->_height - 2) // nếu không xảy ra trường hợp trên và vị trí mặc định được giữ cho tới khi trái banh nằm ở vị trí ngay trên thanh trượt
		{
	...
	if (dest_pos == 0 && ball_pos.y == 3 && (ball_dir == DOWN || ball_dir == DOWNLEFT || ball_dir == DOWNRIGHT)) // nếu vị trí cần chuyển tới là 0 , vị trí của banh đang là cách thanh trượt trên 1 đơn vị và đang di chuyển xuống thì thanh trượt bắt đầu tính toán vị trí cần tới mới 
	{
		...
		else
		{
			...
			{
				int distance_width = this->_width - ball_pos.x; // khoảng cách từ banh tới tường phải 
				...
				else // trường hợp banh di chuyển chéo màn hình có va chạm với tường
				{
					...
					int player_distance_to_right_wall = this->_width - this->_playerBottom->getPos().x; // khoảng cách từ thanh trượt tới tường phải 
					...
					{
						this->_playerBottom->setDir(PRIGHT); // thanh trượt di chuyển sang phải 
						return this->_width - distance; // trả về độ rộng màn hình trừ cho khoảng cách từ tường phải tới vị trí đáp xuống của nó ở tường dưới 
					}
...
int Pong::calculatePosTop(int& dest_pos)
{
	balldirection ball_dir = this->_ball->getDir(); // lấy hướng hiện tại của banh
	point ball_pos = this->_ball->getPos(); // lấy vị trí hiện tại của banh
	if (dest_pos == DEFAULT_POS) // nếu đang là vị trí mặc định
	...
	if (dest_pos == 0 && ball_pos.y == this->_height - 3 && (ball_dir == UP || ball_dir == UPLEFT || ball_dir == UPRIGHT)) // nếu vị trí cần chuyển tới là 0 , vị trí của banh đang là cách thanh trượt dưới 1 đơn vị và đang di chuyển lên thì thanh trượt bắt đầu tính toán vị trí cần tới mới 
	{
		...
		else
		{
			if (ball_dir == UPRIGHT) // trường hợp banh di chuyển lên qua phải 
			{
				...
				else // trường hợp banh di chuyển chéo màn hình có va chạm với tường
				{
					...
					int player_distance_to_right_wall = this->_width - this->_playerTop->getPos().x; // khoảng cách từ thanh trượt tới tường phải 
					if ((pos < 0 && player_distance_to_right_wall > distance) || (pos > 0 && player_distance_to_right_wall > distance) || (pos == 0 && player_distance_to_right_wall > distance)) // xét trường hợp banh nằm bên trái hoặc bên phải hoặc bằng thanh trượt và khoảng cách từ thanh trượt tới tường phải lớn hơn khoảng cách từ tường phải tới vị trí banh đáp xuống
					{
						this->_playerTop->setDir(PRIGHT); // thanh trượt di chuyển sang phải 
						return this->_width - distance; // trả về độ rộng màn hình trừ cho khoảng cách từ tường phải tới vị trí đáp xuống của nó ở tường trên
					}
					if ((pos < 0 && distance >player_distance_to_right_wall) || (pos > 0 && distance > player_distance_to_right_wall) || (pos == 0 && distance > player_distance_to_right_wall)) // xét trường hợp banh nằm bên trái hoặc bên phải hoặc bằng thanh trượt và khoảng cách từ thanh trượt tới tường phải nhỏ hơn khoảng cách từ tường phải tới vị trí banh đáp xuống
					{
						this->_playerTop->setDir(PLEFT); // thanh trượt di chuyển sang trái 
